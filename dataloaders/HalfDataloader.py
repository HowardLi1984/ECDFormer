...
def HalfGetAtomBondAngleDataloader(
    ...
    for itm in ecd_sequences:
        line_num = itm['id'] - 1
        current_smiles = smiles_all[line_num]
        if "@@" not in current_smiles and "@" in current_smiles:
            ## the smiles is single-@
        ...
        elif "@@" in current_smiles:
            ## the smiles is double-@, change it into the single-@
            ...
            chiral_smiles = smiles_all[another_line_num]
            if "@@" in chiral_smiles: print(current_smiles, chiral_smiles); assert 1 == 0
