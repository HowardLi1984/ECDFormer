...
def Construct_dataset(args, dataset, data_index, T1, speed, eluent, column_name):
    graph_atom_bond = []
    graph_bond_angle = []
...
def GetAtomBondAngleDataloader(
        args, dataset_all, smiles_all, index_all, T1_all, 
    ...
    total_graph_atom_bond, total_graph_bond_angle = Construct_dataset(
        ...
        Speed_all, Prop_all, Column_info
    )
    print("Case Before Process = ", len(total_graph_atom_bond), len(total_graph_bond_angle), len(smiles_all))

    ## save the dict smiles: original sequence
    # smiles_seq_results = {}
    # for key, org_seq in ecd_original_sequences.items():
    #     line_num = key-1
    #     if smiles_all[line_num] not in smiles_seq_results.keys(): smiles_seq_results[smiles_all[line_num]] = org_seq
    #     else: assert "error when saving the dict smiles original"
    ...
    #     if smiles_all[another_line_num] not in smiles_seq_results.keys():
    #         temp_ecd = org_seq['ecd']
    #         temp_dict = dict(
    #             wavelengths = org_seq['wavelengths'], 
    #             ecd = [-x for x in temp_ecd],
    #         ) 
    #         smiles_seq_results[smiles_all[another_line_num]] = temp_dict
    #     else: assert "error when saving the dict smiles original" 

    # json.dump(smiles_seq_results, open("/root/workspace/aigc/ChemGNN/dataset/ECD/smiles_org.json", "w"))
    # assert 1 == 0
    ...
    for itm in ecd_sequences:
        ...
        atom_bond_oppo['peak_position'] = torch.tensor([itm['peak_position']])
        atom_bond_oppo['peak_height'] = torch.tensor([itm['peak_height']])
        ...
        dataset_smiles.append(smiles_all[another_line_num])
