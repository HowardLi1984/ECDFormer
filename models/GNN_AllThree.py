...
class BondFloatRBF(torch.nn.Module):
    ...
    def __init__(self, args, embed_dim, rbf_params=None):
        ...
        for name in self.bond_float_names:
            ...
            rbf = RBF(centers.to(args.device), gamma.to(args.device))
...
class GINNodeEmbedding(torch.nn.Module):
    ...
    def forward(self, batched_atom_bond, batched_bond_angle):
        ...
        # print(x.shape, edge_index.shape, edge_attr.shape, edge_index_ba.shape, edge_attr_ba.shape)
...
class GNNAllThree_Model(nn.Module):
    def __init__(self, args, num_tasks=9, num_layers=5, emb_dim=300, residual=False, drop_ratio=0, JK="last", graph_pooling="attention",
        ...
        """
        super(GNNAllThree_Model, self).__init__()
        ...
        self.query_embed = nn.Embedding(9, self.emb_dim)
    ...
    def forward(self, batched_atom_bond, batched_bond_angle, query_padding_mask=None):
        ...
        pred_number_output = self.pred_number_layer(node_summary_feat)
        ...
        if not self.training:
            ...
            peak_position = [[1]*pred_peak_num[i]+[-1]*(9-pred_peak_num[i]) for i in range(len(pred_peak_num))]
        ...
        query_output = output[:, node_feat.size(1):, :]                 # [batch, 9, emb_dim]
        pred_position_output = self.pred_position_layer(query_output)   # [batch, 9, 20]
        pred_height_output = self.pred_height_layer(query_output)       # [batch, 9, 2]

        if self.training:
            return pred_position_output, pred_height_output, pred_number_output
        else: # inference time
            ...
            return pred_position_output, pred_height_output, \
                   torch.clamp(pred_number_output, min=0, max=1e8), \
                   {"attn": attn_weights.to('cpu').tolist(), "attn_mask": attn_mask.to('cpu').tolist()},


def train_GNNAllThree(args, model, device, loader_atom_bond, loader_bond_angle, optimizer):
    model.train()
    loss_accum = 0
...
def eval_GNNAllThree(args, model, device, loader_atom_bond, loader_bond_angle):
    model.eval()
    loss_accum = 0
    ce_loss = torch.nn.CrossEntropyLoss()
    total_loss_pos, total_loss_height, total_loss_num = 0, 0, 0
    ...
    print(f"LOSS: pos={total_loss_pos}, height={total_loss_height}, num={total_loss_num}")  
    return loss_accum / (step + 1)

def test_GNNAllThree(args, model, device, loader_atom_bond, loader_bond_angle, epoch=-1):
    model.eval()
    ...
    with torch.no_grad():
        for step, batch in enumerate(zip(loader_atom_bond, loader_bond_angle)):
            ...
            for i in range(batch_atom_bond.peak_num.size(0)):
                ...
                height_gt_labels = height_gt[i, :int(batch_atom_bond.peak_num[i])].tolist()
                height_pred_labels = torch.argmax(height_pred[i, :int(pred_peak_num[i]), :], dim=1).tolist()

                batch_height_gt.append(height_gt_labels)
                batch_height_pred.append(height_pred_labels)
    ...
    peak_num_acc_list = dict(
        acc = acc / total_case_num,
        acc_1 = acc_1 / total_case_num,
        acc_2 = acc_2 / total_case_num,
        acc_3 = acc_3 / total_case_num,
    )
    ...
    # for i in range(0, len(num_gt_list), args.batch_size): 
    #     if i+args.batch_size >= len(num_gt_list):
    #         tmp_num_rmse = np.sqrt(
    #             mean_squared_error(num_gt_list[i:], num_pred_list[i:]))
    #     else:
    #         tmp_num_rmse = np.sqrt(
    #             mean_squared_error(num_gt_list[i:i+args.batch_size], num_pred_list[i:i+args.batch_size]))
    #     num_rsme_list.append(tmp_num_rmse)
    
    new_pos = sum(pos_results)/len(pos_results)
    num_rmse = sum(num_rmse_list)/len(num_rmse_list)
    height_first = correct_first_num / total_first_num
    height_total = correct_num / total_num

    print("epoch: {}, new_pos = {}, height_first_match = {}, height_all_match = {}, num = {}, num_rmse = {}\n".format(
        epoch, new_pos, height_first, height_total, peak_num_acc_list, num_rmse,
    ))
...
def pred_real_GNNAllThree(args, model, device, loader_atom_bond, loader_bond_angle, epoch=-1):
    model.eval()
